...
- Make the per-job event cap configurable and signal truncation on the timeline API. Both stores are fixed at 10000 events with no flag reaching them, and `tool_call` roughly doubles a tool-heavy run's volume, so a long run now loses its early timeline silently.
