...
| `haybale.yaml` | ConfigMap ×2 + Secret + Deployment + Service | haybale, the git-credential proxy a sandbox reaches through hairpin's sandbox-identity token issuer. See [Deploying haybale](#deploying-haybale). |
